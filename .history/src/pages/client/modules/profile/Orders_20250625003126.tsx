...
import ProfileLayout from '../src//components/Client/ProfileLayout/ProfileLayout';
    ...
    const convertToFacturaData = (pedido: PedidoVentaResponse) => {
        return {
            ...
            items: pedido.detalles?.map(detalle => ({
                nombre: detalle.articulo?.denominacion || 'Producto',
                cantidad: detalle.cantidad || 1,
                precio: detalle.subTotal || 0
            })) || [],
            subtotal: pedido.totalCosto || 0,
            delivery: 0, // Ajustar según tu lógica
            descuento: (pedido.totalCosto || 0) * 0.1, // 10% descuento ejemplo
    ...
    const convertToDetallesData = (pedido: PedidoVentaResponse) => {
        return {
            ...
            items: pedido.detalles?.map(detalle => ({
                ...
                cantidad: detalle.cantidad || 1,
                precio: detalle.subTotal || 0,
                imagen: detalle.articulo?.imagenes?.[0]?.url
            })) || [],
            subtotal: pedido.totalCosto || 0
        };
    };
