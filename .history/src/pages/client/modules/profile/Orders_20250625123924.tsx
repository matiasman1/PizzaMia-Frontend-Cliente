...
interface OrderForTable {
    ...
    fecha: string;
    total: string;
    estado: React.ReactNode; // CORREGIDO: Estado como ReactNode para el span con estilos
    tipo: string;
...
const Orders: React.FC = () => {
    ...
    // CORREGIDO: Convertir pedido a formato de factura con propiedades que SÍ existen
    const convertToFacturaData = (pedido: PedidoVentaResponse) => {
        return {
            ...
            items: pedido.detalles?.map(detalle => {
                // CORREGIDO: Acceder a las propiedades que SÍ existen
    ...
    // CORREGIDO: Convertir pedido a formato de detalles con propiedades que SÍ existen
    const convertToDetallesData = (pedido: PedidoVentaResponse) => {
        return {
            ...
            items: pedido.detalles?.map(detalle => {
                // CORREGIDO: Acceder a las propiedades que SÍ existen
                const articulo = detalle.articuloManufacturado;
                const precio = articulo ? (detalle.cantidad || 1) * (detalle.subTotal || 0) : 0;
    ...
    // CRÍTICO: Convertir pedidos con disposición vertical de botones y estado renderizado
    const convertPedidosToTableData = (pedidos: PedidoVentaResponse[]): OrderForTable[] => {
        return pedidos.map(pedido => {
            ...
            return {
                ...
                // CORREGIDO: Estado como ReactNode ya renderizado
                estado: (
                    <span
                        style={{
        ...
        <ProfileLayout>
            <div className={styles.container}>
                ...
                <div className={styles.tableContainer}>
                    {/* CORREGIDO: Solo usar props que SÍ existen en GenericTable */}
                    <GenericTable
                        data={currentData}
                        columns={columns}
                    />
