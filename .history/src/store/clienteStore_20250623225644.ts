...
import { obtenerClientePorId, obtenerClientePorAuth0, actualizarCliente, obtenerPedidosPorClienteAuth0 } from '../api/clientApi';
...
interface ClienteState {
  ...
  loadingPedidos: boolean; // Corregido: cambiar de false a boolean
  
  // Acciones
  cargarCliente: (id: number) => Promise<void>;
  cargarClientePorAuth0: (auth0Id: string, token: string) => Promise<void>;
  actualizarDatosCliente: (datosCliente: any, token: string) => Promise<void>; // Corregido: cambiar retorno a Promise<void>
    ...
    (set, get) => ({
      ...
      cargarCliente: async (id: number) => {
        try {
          ...
          // Filtrar solo domicilios activos - Corregido: tipar parámetro
          const domiciliosActivos = clienteData.domicilios?.filter((d: DomicilioApi) => d.isActive) || [];
      ...
      // CORREGIDA - Cargar datos del cliente por Auth0 ID usando obtenerClientePorAuth0
      cargarClientePorAuth0: async (auth0Id: string, token: string) => {
        try {
          console.log('Auth0 ID enviado:', auth0Id);
          console.log('Token:', token ? 'Token presente' : 'No token');
          set({ loading: true, error: null });
          
          // Usar obtenerClientePorAuth0 en lugar de getUserById
          const clienteData = await obtenerClientePorAuth0({ auth0Id }, token);
      ...
      // Actualizar datos del cliente - Corregido: no devolver valor
      actualizarDatosCliente: async (datosCliente: any, token: string) => {
        try {
          ...
          // No devolver nada - ya que Promise<void>
        } catch (error) {
