...
interface ClienteState {
  ...
  actualizarDatosCliente: (datosCliente: any, token: string) => Promise<ClienteApi>;
    ...
    (set, get) => ({
      ...
      cargarClientePorAuth0: async (auth0Id: string, token: string) => {
        try {
          ...
          // Filtrar solo domicilios activos
          const domiciliosActivos: DomicilioApi[] = clienteData.domicilios?.filter((d: DomicilioApi) => d.isActive) || [];
