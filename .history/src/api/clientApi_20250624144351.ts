...
// Función para login alternativa
export const postLoginAuth = async (userData: any, token: string): Promise<ClienteApi> => {
    try {
...
// Obtener manufacturados por rubro (para el menú) - versión alternativa
export const obtenerManufacturadosPorRubroMenu = async (
    rubroId: number,
    page: number = 0,
    size: number = 10,
    ...
    try {
        ...
        const filteredContent = response.content.filter(item => {
            ...
            const itemRubroId = item.rubro?.id;
